...
public class SqlParseTest {
    ...
    public void parseTest() {
        String sql = new StringBuilder().append("select a.id, CASE WHEN a.id <= 500 THEN '1' else '2' end as id2, ").append("concat(a.name,'_test') as name, a.role_id, b.name as role_name, c.labels from user a ").append("left join role b on a.role_id=b.id ").append("left join (select user_id, group_concat(label,',') as labels from user_label ").append("group by user_id) c on c.user_id=a.id").toString();
        ...
        fieldItems.forEach(
            fieldItem -> Assert.assertEquals("c.labels", new StringBuilder().append(fieldItem.getOwner()).append(".").append(fieldItem.getFieldName()).toString()));
