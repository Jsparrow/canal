...
public interface CanalEventStore<T> extends CanalLifeCycle, CanalStoreScavenge {
     ...
     */
    void put(List<T> data) throws InterruptedException;
     ...
     */
    boolean put(T data, long timeout, TimeUnit unit) throws InterruptedException;
     ...
     * 获取指定大小的数据，阻塞等待其操作完成
     */
    Events<T> get(Position start, int batchSize) throws InterruptedException;
     ...
     * 根据指定位置，获取一个指定大小的数据
     */
    Events<T> tryGet(Position start, int batchSize);
