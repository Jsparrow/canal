    ...
    private final BigDecimal getDecimal0(final int begin, final int intg, final int frac, final int intg0,
        ...
        if (intg0x != 0) {
            ...
            if (x < 0 || x >= powers10[intg0x + 1]) {
                throw new IllegalArgumentException(new StringBuilder().append("bad format, x exceed: ").append(x).append(", ").append(powers10[intg0x + 1]).toString());
            }
            if (x != 0 /* !digit || x != 0 */) {
        ...
        for (final int stop = from + intg0 * SIZE_OF_INT32; from < stop; from += SIZE_OF_INT32) {
            ...
            x ^= mask;
            if (x < 0 || x > DIG_MAX) {
                throw new IllegalArgumentException(new StringBuilder().append("bad format, x exceed: ").append(x).append(", ").append(DIG_MAX).toString());
            }
            if (x != 0) {
                if (mark < pos) {
